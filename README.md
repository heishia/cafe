# Naver Cafe Comment Helper

네이버 OAuth와 카페 API를 사용해 외부에서 만든 댓글 초안을 저장하고, 운영자가 승인한 댓글만 게시하는 보조 서버입니다.
...
- `POST /drafts/comment`: 외부에서 만든 댓글 내용을 초안으로 저장
- `GET /drafts`: 초안 목록 조회
- `GET /drafts/:id`: 초안 상세 조회
- `POST /drafts/:id/approve`: 사람이 검토한 초안 승인
- `POST /drafts/:id/reject`: 초안 반려
- `POST /drafts/:id/publish`: 승인된 댓글 초안만 네이버 카페에 게시
...
1. 외부 AI 또는 사람이 만든 댓글 내용을 서버에 초안으로 저장합니다.
2. 초안은 `data/drafts.json`에 `pending` 상태로 저장됩니다.
3. 운영자가 내용을 검토하고 필요하면 수정하면서 승인합니다.
4. 승인된 댓글 초안만 `publish` API로 네이버 카페에 게시됩니다.
...
curl -X POST http://localhost:3000/drafts/comment \
  ...
  -d "{\"articleTitle\":\"바이브코딩 질문\",\"articleContent\":\"원문 본문\",\"articleId\":\"123\",\"content\":\"OpenCl로 만든 최종 댓글 초안\",\"accountKey\":\"2\"}"
```
...
curl -X POST http://localhost:3000/drafts/{draftId}/approve \
  -H "Content-Type: application/json" \
  -H "x-api-key: $NAVER_SERVER_API_KEY" \
  -d "{\"reviewer\":\"admin\",\"content\":\"검토 후 수정한 최종 댓글\"}"
```
